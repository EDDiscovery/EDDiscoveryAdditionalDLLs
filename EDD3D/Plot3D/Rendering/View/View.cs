...
namespace EDD3D.Plot3D.Rendering.View
{
	...
	public class View
	{
		...
		public void Zoom(float factor, bool updateView)
		{
			...
			EDD3D.Maths.Scale scale = null;
			if ((zmin < zmax)) {
				scale = new EDD3D.Maths.Scale(zmin, zmax);
			} else {
				// forbid to have zmin = zmax if we zoom in
				if ((factor < 1)) {
					scale = new EDD3D.Maths.Scale(center, center);
				}
			}
			if ((scale != null)) {
		...
		public void ZoomX(float factor, bool updateView)
		{
			...
			EDD3D.Maths.Scale scale = null;
			if ((min < max)) {
				scale = new EDD3D.Maths.Scale(min, max);
			} else {
		...
		public EDD3D.Maths.Scale Scale {
			get { return new EDD3D.Maths.Scale(this.Bounds.zmin, this.Bounds.zmax); }
			set { setScale(value, true); }
		}

		public void setScale(EDD3D.Maths.Scale scale, bool notify)
		{
			BoundingBox3d bounds = this.Bounds;
			bounds.zmin = scale.Min;
			bounds.zmax = scale.Max;
		...
		public void RenderSceneGraph(bool light)
		{
			...
			Transform.Transform transform = new Transform.Transform(new EDD3D.Plot3D.Transform.Scale(_scaling));
