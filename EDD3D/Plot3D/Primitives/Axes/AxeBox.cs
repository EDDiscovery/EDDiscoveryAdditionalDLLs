...
using EDD3D.Plot3D.Text;
using EDD3D.Plot3D.Text.Align;
using EDD3D.Plot3D.Text.Renderers;
	...
	public class AxeBox : IAxe
	{
		...
		public void Draw(Rendering.View.Camera camera)
		{
			...
			if (_layout.FaceDisplayed) {
				...
				// handle stippling
				drawCube(OpenTK.Graphics.OpenGL.RenderingMode.Render);
			...
			if ((_xrange > 0 & _layout.XTickLabelDisplayed)) {
				...
				if ((((_view != null) && _view.ViewMode == EDD3D.Plot3D.Rendering.View.Modes.ViewPositionMode.TOP))) {
		...
		/// </summary> 
		public void drawCube(OpenTK.Graphics.OpenGL.RenderingMode mode)
		{
			for (int q = 0; q <= 5; q++) {
				if (mode == OpenTK.Graphics.OpenGL.RenderingMode.Feedback) {
					GL.PassThrough(q);
				}
